...
const Login: React.FC<{ onLoginSuccess: () => void }> = ({ onLoginSuccess }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '80vh' }}>
      {/* GRID LOGIC MOVED TO CSS CLASS */}
      <div className="login-grid" style={{ marginTop: '60px' }}>
        <div>
          <h1 className="login-title" style={{ fontSize: '56px', fontWeight: 800, marginBottom: '24px', lineHeight: 1, letterSpacing: '-2px' }}>
            Use Your Telegram as your<br/><span className="logo-text" style={{ fontSize: '64px' }}>Personal Cloud.</span>
          </h1>
        ...
        <div className="glass-card" style={{ padding: '40px', borderRadius: '28px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
          <h3 style={{ marginTop: 0, fontWeight: 800 }}>Log in to your Telegram</h3>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '24px' }}>Enter your number with +880 to continue.</p>
          
          {step === 'phone' ? (
            <div style={{ width: '100%' }}>
              <input type="text" placeholder="+8801xxxxxxxxx" value={phone} onChange={e => setPhone(e.target.value)}
