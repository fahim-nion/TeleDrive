...
const MediaViewer: React.FC<any> = ({ files, currentIndex, onClose, onIndexChange, onDelete, onDownload }) => {
  ...
  useEffect(() => {
    files.forEach(async (file: any) => {
      if (file.thumbnail && !thumbs[file.messageId]) {
        const tUrl = await getThumbnail(file.thumbnail);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000', zIndex: 3000, display: 'flex', flexDirection: 'column' }}>
      {/* HEADER: Reduced padding from 20px to 10px to prevent button cutoff */}
      <div style={{ padding: '12px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.6)', zIndex: 3010 }}>
        <button onClick={onClose} style={{ color: 'white', background: 'rgba(255,255,255,0.1)', border: 'none', padding: '8px 14px', borderRadius: '100px', fontWeight: '800', cursor: 'pointer', fontSize: '10px', flexShrink: 0 }}>CLOSE</button>
        
        {/* BUTTON GROUP: Reduced gap from 8px to 6px */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          ...
          <button onClick={() => onDownload(f)} style={{ background: 'var(--primary)', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '10px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <IconDownload size={14} />
          </button>
          
          <button onClick={() => { if(confirm('Delete?')) onDelete(f.messageId); }} style={{ background: '#EF4444', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '10px', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <IconTrash size={14} />
          </button>
        </div>
      </div>
